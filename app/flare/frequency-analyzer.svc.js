...
function FrequencyAnalyzer(MScheduler, AudioData, FrequencyRanges, MaxFrequency, AudioPlayer) {
    const results = {
        ...
        // This prevents a large slice of the visualizer from being empty early in
        // the song or for songs that smaller range of data
        dataLimit: 0,
        maxRangeIndices: [],
    ...
    function analyzerFrequencyData(frequencies, outResults) {
        let loudness = 0;
        let frequency = 0;
        let dataLimit = outResults.dataLimit;
        let currentRangeIndex = 0;
        const maxRangeLoudness = outResults.maxRangeLoudness;
        const maxRangeIndices = outResults.maxRangeIndices;
        const frequencyInterval = MaxFrequency / frequencies.length;
        let avgLoudness = 0;
