...
function asyncRequestFactory(Thread, $q, Path) {
    ...
    const asyncScript = 'asyncHttpRequest';
    ...
    function RequestPool(size, base)    {
        const threads      = []; // Threads available to execute requests
        const requestQueue = new PriorityQueue(); // queue of pending requests awaiting threads
         ...
         */
        this.send = function send(config) {
    ...
    const defaultPool = createRequestPool(1, Path.relativeBase);
