...
function Geometry(MM) {
    class Transform {
         ...
         * @returns {Transform}
         */
        timeTranslate(velocity, deltaTime) {
    ...
    class Mesh {
        ...
        static buildVertexBuffer(verts) {
         ...
         * @returns {Vector3[]}
         */
        static buildNormals(verts, indices) {
            ...
            for (let i = 0; i < indices.length; i += 3) {
                ...
                // const angle = Math.acos(ab.normalize().dot(ac.normalize()))
                // * Math.sign(ab.cross(ac).dot(unitNormal));
