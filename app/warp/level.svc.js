...
require('angular').module('pulsar.warp').service('warp.Level', [
    'warp.State',
 ...
 * @constructor
 */
function Level(State, WaveformAnalyzer, Bar, MM) {
    const self = this;
    this.barQueue = [0];
    this.barsVisible = 55;
    ...
    let elapsed = 0; // elapsed time since last bar was rendered
    ...
    const frequencySamples = 10; // how many waveform frequencies to average
    ...
    this.getLoudness = relativeIndex => (self.warpField[self.sliceIndex + relativeIndex] || {}).loudness || 0;
    ...
    this.update = (dt) => {
        ...
        while (elapsed > self.timeStep / 1000) {
            ...
            const waveform = WaveformAnalyzer.getMetrics();
            // Create a new bar
        ...
        const velocity = (Bar.scale.z * self.barQueue[2].speed + Bar.margin) / self.timeStep;
