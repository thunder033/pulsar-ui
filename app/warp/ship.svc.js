 ...
 */
require('angular').module('pulsar.warp').service('warp.Ship', [
    MDT.Scheduler,
    MDT.Camera,
    MDT.Math,
    MDT.Keyboard,
...
function Ship(MScheduler, MCamera, MM, MKeyboard, MKeys, Geometry) {
    const self = this;
    const velocity = MM.vec3(0);
    let destLane = 0;
    const moveSpeed = 0.0045;
    const laneWidth = 1.15;
    ...
    function hasReachedLane() {
        const lanePos = (destLane - 1) * laneWidth;
    ...
    function getLaneCoord() {
        const relPos = (tShip.position.x + laneWidth) % laneWidth;
    ...
    function isInBounds(moveDistance) {
        const minBound = -laneWidth - moveDistance;
        const maxBound = +laneWidth + moveDistance;
    ...
    MScheduler.schedule((dt) => {
        ...
        if (activeCtrl !== null && MKeyboard.isKeyDown(activeCtrl) && isInBounds(moveSpeed * dt)) {
            ...
            let rightBound = 0; // figure out which lane ship is left of
            while ((rightBound - 1) * laneWidth <= tShip.position.x) {
                rightBound++;
            }
        ...
        if (bankPct !== 0 && velocity.len2() === 0) {
            const sign = MM.sign(bankPct);
            bankPct -= bankRate * dt * sign;
            bankPct = MM.clamp(bankPct, -1, 1);

            const newSign = MM.sign(bankPct);
            if (newSign !== sign) {
                bankPct = 0;
            }
        }
        ...
        MScheduler.draw(() => {
            ...
            // MCamera.drawShape(Shapes.Triangle,
            // MM.vec3(tShip.position.x, 0, tShip.position.z), shipWidth * Math.cos(bankAngle), 10, 0);
        }, this.priority);
    }, this.priority);
}
